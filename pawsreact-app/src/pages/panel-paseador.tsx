...
import {
  IonPage, IonHeader, IonToolbar, IonTitle, IonContent, IonText, IonBadge,
  IonButton, IonCard, IonCardContent, IonCardHeader, IonCardSubtitle, IonCardTitle,
  IonItem, IonAlert, IonSpinner, IonList, IonLabel
} from "@ionic/react";
...
const PanelPaseador: React.FC = () => {
  ...
  const [misPaseos, setMisPaseos] = useState<Paginated<PaseoListItem> | null>(null);
  const [disponibles, setDisponibles] = useState<Paginated<PaseoListItem> | null>(null);
  const [loading, setLoading] = useState(true);
  const [workingId, setWorkingId] = useState<number | null>(null);
  const [alert, setAlert] = useState<{open:boolean; header?:string; message?:string}>({open:false});
  ...
  try {
    ...
    setAlert({open:true, header:"Error", message: e?.response?.data?.error || e?.message || "No se pudieron cargar los paseos"});
  } finally {
    setLoading(false);
  }
};
  ...
  const tomarPaseo = async (idPaseo: number) => {
    try {
      ...
      await cargar();
      setAlert({open:true, header:"Listo", message:"Paseo asignado a ti."});
    } catch (e: any) {
      setAlert({open:true, header:"Error", message: e?.response?.data?.error || e?.message || "No se pudo tomar el paseo"});
    } finally {
      setWorkingId(null);
    }
  };
  ...
  const iniciar = async (idPaseo: number) => {
    try {
      ...
      await cargar();
      setAlert({open:true, header:"En curso", message:"Comenzaste el paseo."});
    } catch (e: any) {
      setAlert({open:true, header:"Error", message: e?.response?.data?.error || e?.message || "No se pudo iniciar el paseo"});
    } finally {
      setWorkingId(null);
    }
  };
  ...
  const finalizar = async (idPaseo: number) => {
    try {
      ...
      await cargar();
      setAlert({open:true, header:"Finalizado", message:"Paseo finalizado correctamente."});
    } catch (e: any) {
      setAlert({open:true, header:"Error", message: e?.response?.data?.error || e?.message || "No se pudo finalizar el paseo"});
    } finally {
      setWorkingId(null);
    }
  };

  const fmtFecha = (iso: string) => { try { return new Date(iso).toLocaleDateString(); } catch { return iso; } };
  const fmtHora  = (iso: string) => { try { return new Date(iso).toLocaleTimeString([], {hour:"2-digit", minute:"2-digit"}); } catch { return iso; } };
  ...
  const renderAccionAsignado = (p: PaseoListItem) => {
    ...
    if (p.estado === "PENDIENTE" || p.estado === "ACEPTADO") {
      return (
        ...
        >
          {workingId === p.idPaseo ? <IonSpinner name="dots" /> : "Iniciar paseo"}
        </IonButton>
      );
    }
    if (p.estado === "EN_CURSO") {
      return (
        <IonButton
          ...
          disabled={workingId === p.idPaseo}
        >
          {workingId === p.idPaseo ? <IonSpinner name="dots" /> : "Finalizar paseo"}
        </IonButton>
      );
    }
    ...
    <IonPage>
      <IonHeader>
        <IonToolbar color="selective-yellow">
          <IonTitle color="prussian-blue" className="coffeecake">Paws At Route</IonTitle>
        </IonToolbar>
      ...
      <IonContent className="ion-padding" fullscreen>
        <IonText color="prussian-blue">
          ...
          <h1 style={{ fontWeight: "bold" }}>¡Hola, {nombrePaseador || "Paseador"}!</h1>
        </IonText>
        ...
        {loading ? (
          <div className="ion-text-center ion-margin-vertical">
            ...
            <IonText><p>Cargando paseos…</p></IonText>
          </div>
          ...
          misPaseos.items.map((p) => (
            <IonCard key={p.idPaseo}>
              <IonCardHeader className="ion-text-center">
                ...
                <IonCardSubtitle>
                  Dueño: {p.duenio ? `${p.duenio.nombre} ${p.duenio.apellido}` : "—"}
                </IonCardSubtitle>
              </IonCardHeader>
              <IonCardContent>
                <IonList lines="none">
                  <IonItem><IonLabel><b>Fecha:</b> {fmtFecha(p.fecha)}</IonLabel></IonItem>
                  <IonItem><IonLabel><b>Hora:</b> {fmtHora(p.hora)}</IonLabel></IonItem>
                  <IonItem><IonLabel><b>Duración:</b> {p.duracion} min</IonLabel></IonItem>
                  <IonItem><IonLabel><b>Dirección:</b> {p.lugarEncuentro}</IonLabel></IonItem>
                  <IonItem>
  <IonLabel>
    <b>Estado:</b>{" "}
    <IonBadge color={estadoColor[p.estado as Estado]}>
      {estadoLabel[p.estado as Estado]}
          ...
          <IonText><p><b>No hay paseos asignados</b></p></IonText>
        )}
        ...
        {loading ? null : (disponibles && disponibles.items.length ? (
              ...
              <IonCardContent>
                ...
                <div className="ion-text-center ion-margin-top">
                  <IonButton
                    ...
                    disabled={workingId === p.idPaseo}
                  >
                    {workingId === p.idPaseo ? <IonSpinner name="dots" /> : "Tomar paseo"}
                  </IonButton>
                </div>
              </IonCardContent>
            </IonCard>
          ...
          <IonText><p>No hay paseos disponibles por ahora.</p></IonText>
        ))}
