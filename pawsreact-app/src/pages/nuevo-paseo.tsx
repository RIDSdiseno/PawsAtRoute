...
const NuevoPaseo: React.FC = () => {
  ...
  const validar = (): string | null => {
    ...
    if (selMascotas.length > MAX_MASCOTAS) return `Solo puedes seleccionar hasta ${MAX_MASCOTAS}.`;
    if (!fechaStr || !horaStr) return "Debes elegir fecha y hora.";
    if (!duracionMin) return "Debes elegir la duración.";
    if (!lugar || lugar.trim().length < 10) return "Ingresa una ubicación válida (mín. 10 caracteres).";
    return null;
  };
  ...
  const u = Auth.getUser();
  if (!u?.idUsuario) {
    setAlert({ open: true, header: "Sesión", message: "Debes iniciar sesión nuevamente." });
    router.push("/login", "root");
    return;
  }
  ...
  try {
    ...
    const msg = e?.response?.data?.error || e?.message || "No se pudo publicar el paseo";
    setErr(msg);
      ...
      <IonContent className="ion-padding" fullscreen>
        <IonCard>
          <IonCardHeader className="ion-text-center">
            <IonCardTitle color="prussian-blue">Solicitud de paseo</IonCardTitle>
          ...
          <IonCardContent>
            {loading ? (
              <div className="ion-text-center ion-margin-vertical">
                ...
                <IonText><p>Cargando tus mascotas…</p></IonText>
              </div>
            ) : (
              <>
                {/* Mascotas */}
                <IonList lines="none">
                    ...
                    {mascotas.length ? (
                      <IonList>
                        {mascotas.map((m) => {
                          ...
                          const disabled = !checked && selMascotas.length >= MAX_MASCOTAS;
                          return (
                            <IonItem key={m.idMascota}>
                              <IonCheckbox
                                ...
                                checked={checked}
                                disabled={disabled}
                                onIonChange={(e) => toggleMascota(m.idMascota, !!e.detail.checked)}
                              />
                              <IonLabel>
                      ...
                      <IonNote color="medium">
                        No tienes mascotas registradas. Registra una antes de publicar un paseo.
                      </IonNote>
                    )}

                    <IonNote slot="helper">
                      Puedes seleccionar hasta {MAX_MASCOTAS} por paseo. ({selMascotas.length}/{MAX_MASCOTAS})
                    </IonNote>
                  </IonItem>
                  ...
                  <IonItem>
                    ...
                    <IonModal keepContentsMounted>
                      <IonDatetime
                        ...
                        onIonChange={(e) => setFechaISO(String(e.detail.value || ""))}
                      />
                    </IonModal>
                    <IonNote slot="helper">{fechaISO ? "Seleccionado" : "Elige fecha y hora"}</IonNote>
                  </IonItem>

                  {/* Duración */}
                  <IonItem>
                    <IonLabel position="stacked" color="prussian-blue">Duración</IonLabel>
                    <IonRadioGroup value={duracionMin} onIonChange={(e) => setDuracionMin(String(e.detail.value))}>
                      <IonItem>
                        <IonRadio value="30">
                          30 minutos <IonChip color="success">$10.000 CLP</IonChip>
                        </IonRadio>
                      </IonItem>
                      <IonItem>
                        <IonRadio value="60">
                          1 hora <IonChip color="success">$15.000 CLP</IonChip>
                        </IonRadio>
                      </IonItem>
                      <IonItem>
                        <IonRadio value="90">
                          1 hora y 30 minutos <IonChip color="success">$20.000 CLP</IonChip>
                        </IonRadio>
                      </IonItem>
                      <IonItem>
                        <IonRadio value="120">
                          2 horas <IonChip color="success">$25.000 CLP</IonChip>
                        </IonRadio>
                      </IonItem>
                    </IonRadioGroup>
                  </IonItem>
                  ...
                  {/* Ubicación */}
                  <IonItem>
                    <IonLabel position="stacked" color="prussian-blue">Ubicación</IonLabel>
                    <IonInput
                      required
                      minlength={10}
                      maxlength={100}
                  ...
                  {/* Método de pago (opcional → lo guardamos en notas) */}
                  <IonItem>
                    <IonLabel position="stacked" color="prussian-blue">Método de pago</IonLabel>
                    <IonSelect
                      placeholder="Selecciona un método"
                      value={metodoPago}
                      onIonChange={(e) => setMetodoPago(String(e.detail.value || ""))}
                    >
                  ...
                  {/* Notas opcionales */}
                  <IonItem>
                    <IonLabel position="stacked" color="prussian-blue">Notas (opcional)</IonLabel>
                    <IonInput
