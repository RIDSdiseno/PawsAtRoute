import {
  ...
  IonAccordionGroup, IonAccordion, IonItem, IonLabel, IonBadge, IonSpinner
} from "@ionic/react";
import { useEffect, useMemo, useState } from "react";
import { useIonRouter } from "@ionic/react";
import { Auth } from "../services/auth";
import { listMisPaseosComoPaseador, type PaseoListItem, type Paginated } from "../services/api";
...
const tarifaPorMinuto = (min: number) => {
  switch (min) {
    ...
    case 90:  return 20000;
    case 120: return 25000;
    default:  return 0; // si otro valor, ajusta acá
  }
};
...
const EstadisticasPaseador: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (u.rol !== "PASEADOR") { router.push("/tabs/tab1", "root"); return; }
    ...
    <IonPage>
      ...
      <IonContent className="ion-padding" fullscreen>
        <IonText color="prussian-blue">
          ...
          <h1 style={{ fontWeight: "bold" }}>Mis estadísticas</h1>
        </IonText>
        <IonText>
          <p>Aquí puedes ver tus estadísticas como paseos completados e historial de paseos.</p>
        </IonText>
        ...
        <IonCard className="ion-text-center">
          ...
          <IonCardContent>
            {loading ? <IonSpinner name="dots" /> : <IonText><h1>${saldo.toLocaleString("es-CL")}</h1></IonText>}
          </IonCardContent>
        </IonCard>
        ...
        <IonCard className="ion-text-center">
          <IonCardHeader>
            <IonCardTitle color="prussian-blue">Paseos completados</IonCardTitle>
          </IonCardHeader>
          <IonCardContent>
            {loading ? <IonSpinner name="dots" /> : <IonText><h1>{completados}</h1></IonText>}
          </IonCardContent>
        </IonCard>
        ...
        <IonCard className="ion-text-center">
          ...
          <IonCardContent>
            {loading ? (
              <div className="ion-text-center"><IonSpinner name="dots" /></div>
            ) : completados === 0 ? (
              <IonText><p>No tienes paseos finalizados todavía.</p></IonText>
            ) : (
              <IonAccordionGroup>
                {items.map((p) => (
                  <IonAccordion key={p.idPaseo} value={`p-${p.idPaseo}`}>
                    ...
                    <div slot="content" className="ion-padding">
                      <IonItem lines="none"><IonLabel><b>Hora:</b> {fmtHora(p.hora)}</IonLabel></IonItem>
                      <IonItem lines="none"><IonLabel><b>Duración:</b> {p.duracion} min</IonLabel></IonItem>
                      <IonItem lines="none"><IonLabel><b>Dirección:</b> {p.lugarEncuentro}</IonLabel></IonItem>
                      <IonItem lines="none"><IonLabel><b>Dueño:</b> {p.duenio ? `${p.duenio.nombre} ${p.duenio.apellido}` : "—"}</IonLabel></IonItem>
                    </div>
                  </IonAccordion>
                ))}
              </IonAccordionGroup>
